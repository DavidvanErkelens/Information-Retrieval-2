    ...
    for l in labels:
        ...
        if l in pred_labels:
            g2_scores = [f1_score(np.where(y_true == c, 1, 0), 
                                  np.where(y_pred == l, 1, 0)) for c in pred_labels]
...
def topic_stats(embeddings, y_true, num_classes):
    """ Computes topic cluster statistics for document vectors
    Param:
        embeddings: (D, B, C) tensor, where D is num documents, BxD final embedding
    ...
    def score(y_true, y_pred, name):
        ...
        s = [get_avg_f1(y_true, y_pred), adjusted_rand_score(y_true, y_pred), 
              adjusted_mutual_info_score(y_true, y_pred),normalized_mutual_info_score(y_true, y_pred),
              accuracy_score(y_true, topic_act)]
